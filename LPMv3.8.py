...
for INS in config:
                    ...
                    for s in surgeon:
                        ...
                        for a in second:
                            if x[(p,o,s,a,t,d)].solution_value == 1:
                                ...
                                if dfPatient.iloc[p]["nombre"] not in pacientes_atend:
                                    pacientes_atend.append(dfPatient.iloc[p]["nombre"]);
    ...
    dfPROG.to_excel(f"./Output/LPM/{entrada}_Schedule_v{version}p{patient_code}s{nSurgeons}d{nDays}.xlsx", index=False);
    ...
    print(fichas_ocup)
    dict_sol["Avg Fichas"] = [round(np.mean([i for i in fichas_ocup if i != 0]), 1)];
    
    #list_sol[9] = str(round(np.std(fichas_ocup),1));
    dict_sol["Std Fichas"] = [round(np.std([i for i in fichas_ocup if i != 0]), 1)];
    
    print(cirujanos_atend_num)
    #list_sol[10] = str(round(np.mean(cirujanos_atend_num),1));
    dict_sol["Avg Cirug"] = [round(np.mean([i for i in cirujanos_atend_num if i != 0]), 1)];
    
    #list_sol[11] = str(round(np.std(cirujanos_atend_num),1));
    dict_sol["Std Cirug"] = [round(np.std([i for i in cirujanos_atend_num if i != 0]), 1)];
    
    for i in range(nSurgeons):
        if cirujanos_atend_num[i] != 0:
            ratios[i] = round(fichas_ocup[i]/cirujanos_atend_num[i], 1);
            
    #list_sol[12] = str(round(np.mean(ratios),1));
    dict_sol["Avg Ratio"] = [round(np.mean([i for i in ratios if i != 0]), 1)];
    
    #list_sol[13] = str(round(np.std(ratios),1)) + "\n";
    dict_sol["Std Ratio"] = [round(np.std([i for i in ratios if i != 0]), 1)];
