...
def load_data_and_config():
    ...
    global patient,surgeon,second,room,day,nSlot,nRooms,slot,nFichas,OT,AOR,COIN,Ex,I,SP,dictCosts
    ...
    extras=[]
    def load_OR_dist(sheet):
        df_=pd.read_excel("../input/DIST_OR_EXT.xlsx",sheet_name=sheet,converters={'n°':int},index_col=[0]).astype(str).values.tolist(); ext=[]
        for i in range(len(df_)): ext.append(df_[i].copy())
        for i in range(len(df_[0])):
            p=df_[0][i].split(";"); df_[0][i]=p[0]; ext[0][i]=p[1:]
            p2=df_[1][i].split(";"); df_[1][i]=p2[0]; ext[1][i]=p2[1:]
        return df_,ext
    dfdisORA,extA=load_OR_dist('A'); dfdisORB,extB=load_OR_dist('B')
    dfdisORC,extC=load_OR_dist('C'); dfdisORD,extD=load_OR_dist('D')
    extras.extend([extA,extB,extC,extD])
    ...
    patient=list(range(nPatients)); surgeon=list(range(nSurgeons)); second=list(range(nSurgeons))
    room=list(range(len(dfRoom))); day=list(range(nDays)); nSlot=16; nRooms=len(room); slot=list(range(nSlot))
    ...
    for s in surgeon:
        for a in second: COIN[(s,a)]=1 if dfSurgeon.iloc[s][0]==dfSecond.iloc[a][0] else 0
    dictCosts={}; num_ext=2; extra=[]; dfExtra=[]
    for i in range(num_ext):
        aux=pd.read_excel("../input/AFFINITY_EXT.xlsx",sheet_name='Extra'+str(i+1),converters={'n°':int},index_col=[0]); extra.append(len(aux)); dfExtra.append(aux)
    ...
    def bType(e): 
        for i in range(len(dfType)):
            if e==dfType.iloc[i][0]: return i
    for p in patient:
        for s in surgeon:
            if bType(dfPatient.iloc[p]["especialidad"])==bType(dfSurgeon.iloc[s][9]): SP[p][s]=1
