...
for INS in range(rango[0],rango[1]):
    ...
    for s in surgeon:
        for d in day:
            CPAffiDia.append(dfdisAffiDiario.iloc[d][s+1]);
    CPAffiDia.extend([0 for i in range(len(surgeon)*len(day)*10)]);
            
    CPAffiBloq = [];
    for s in surgeon:
        for t in range(len(slot)//4):
    ...
    F = [[[integer_var(name="F_"+str(p)+str(s)+str(d), min=0) for d in range(nDay + 1)] 
          for s in range(len(surgeon)+1)] for p in patient];
    
    F_aux = [[integer_var(name="Faux_"+str(s)+str(d),min=0) for d in range(nDay + 1)] for s in range(len(surgeon)+1)];
    
    fichas = int(instancia[INS][4]);
    ...
    mdl.add(maximize(sum(1000 * I[(p,d)] * C[p][d] for p in patient for d in day)
                    - sum(F_aux[s][d] * (nDay//d) for s in surgeon for d in range(1,nDay+1))));
    ...
    for s in surgeon:
        for d in day:
            for o in room:
                for t in range(len(slot)//4):
                    sum_puntaje = int(sum(Ex[i][(s, int(extras[o][t][d][i]) - 1)] for i in range(num_ext)))
        ...
        for s in surgeon:
            for d in range(1,nDay+1):
                mdl.add(mdl.if_then(mdl.logical_and(M[p] == s, mdl.logical_and(T[p]//len(slot) >= d-1, T[p]//len(slot) < d)), 
                                    ...
                                    mdl.element(CPAffiEx, M[p]*len(day)*len(room)*2 + (T[p] // len(slot))*len(room)*2 + O[p]*2 + (T[p]%len(slot))//4) + 
                                    mdl.element(CPAffiBloq, M[p] * len(slot)//4 + (T[p] % len(slot))//4)))
    ...
    if Potencia == 1:
        solucion = mdl.solve(TimeLimit=1800,Workers=1,RelativeOptimalityTolerance=0.00000000000001);
    elif Potencia == 2:
        solucion = mdl.solve(TimeLimit=3600,RelativeOptimalityTolerance=0.00000000000001);
    ...
    if solucion.is_solution():
        ...
        def escribir(nuevo,txt):
            ...
            txt.append(nuevo);
            return txt;
        with open(carpeta + "TXT/CPM/" + "CPM_C" + ".txt","r") as a:
            texto = a.readlines();
            for i in range(len(texto)):
                texto[i] = texto[i].split(" ");
        with open(carpeta + "TXT/CPM/" + "CPM_C" + ".txt","w") as a:
            a.write("");
        with open(carpeta + "TXT/CPM/" + "CPM_C" + ".txt","a") as a:
    ...
    for p in patient:
        ...
        if m < len(surgeon):
            ...
            #print(CPAffiEx[m*len(day)*len(room)*2 + (val//len(slot))*len(room)*2 + o*2 + (val%len(slot))//4]);
            lista_p.append(p);
        else:
            #print(f"Sol var O[{p}] = {solucion.get_solution().get_value(O[p])}");
            #print(f"Sol var T[{p}] = {solucion.get_solution().get_value(T[p])}");
            #print(f"Sol var M[{p}] = {solucion.get_solution().get_value(M[p])}");
            #print(f"Sol var S[{p}] = {solucion.get_solution().get_value(S[p])}");
            #print(m*len(day)*len(room)*2 + (val//len(slot))*len(room)*2 + o*2 + (val%len(slot))//4, len(CPAffiEx))
            #print(CPAffiEx[m*len(day)*len(room)*2 + (val//len(slot))*len(room)*2 + o*2 + (val%len(slot))//4]);
            pass;
    ...
    for s in surgeon:
        fichas_ocup[s] = fichas*nDay - solucion.get_solution().get_value(F_aux[s][nDay])
        for d in range(len(day)+1):
            print(f"s: {s}, d: {d}, sol: {solucion.get_solution().get_value(F_aux[s][d])}")
    ...
    dfSTATS = pd.DataFrame([[dfSurgeon.iloc[s][0] + ": ",str(sum(solucion.get_solution().get_value(F[p][s][nDay]) for p in patient))] for s in surgeon]);
    print(dfSTATS);
    ...
    from os.path import exists;
    existe_txt = exists(carpeta + "TXT/CPM/" + "CPM_C_" + str(fichas) + ".txt");
    print(existe_txt);
    if existe_txt == False:
        aux = open(carpeta + "TXT/CPM/" + "CPM_C_" + str(fichas) + ".txt","w");
        aux.close();
    ...
    def escribir(nuevo,txt):
        ...
        txt.append(nuevo);
        return txt;
    with open(carpeta + "TXT/CPM/" + "CPM_C_" + str(fichas) + ".txt","r") as a:
        texto = a.readlines();
        for i in range(len(texto)):
            texto[i] = texto[i].split(" ");
    with open(carpeta + "TXT/CPM/" + "CPM_C_" + str(fichas) + ".txt","w") as a:
        a.write("");
    with open(carpeta + "TXT/CPM/" + "CPM_C_" + str(fichas) + ".txt","a") as a:
        texto = escribir(list_sol,texto);
        for i in texto:
            i = " ".join(i);
            a.write(i)
