...
for INS in range(rango[0],rango[1]):
    ...
    for p in patient:
        for s in surgeon:
            mdl.add(mdl.if_then(SP[(p,s)] == 0, M[p] != s));
    ...
    if solucion.is_solution():
        ...
        existe_txt = exists(carpeta + "TXT/CPM/" + "CPM_A" + ".txt");
        print(existe_txt);
        if existe_txt == False:
            aux = open(carpeta + "TXT/CPM/" + "CPM_A" + ".txt","w");
            aux.close();
        ...
        def escribir(nuevo,txt):
            ...
            txt.append(nuevo);
            return txt;
        with open(carpeta + "TXT/CPM/" + "CPM_A" + ".txt","r") as a:
            texto = a.readlines();
            for i in range(len(texto)):
                texto[i] = texto[i].split(" ");
        with open(carpeta + "TXT/CPM/" + "CPM_A" + ".txt","w") as a:
            a.write("");
        with open(carpeta + "TXT/CPM/" + "CPM_A" + ".txt","a") as a:
    ...
    writer = ExcelWriter(carpeta + "Resultados/Constraint Programming/CPM A/" + "CPM_A_" +instancia[INS][0]+"_"+instancia[INS][1]+"_"+instancia[INS][2]+"_"+instancia[INS][3]+".xlsx");
